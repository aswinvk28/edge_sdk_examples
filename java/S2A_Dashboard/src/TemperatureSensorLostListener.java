...
public class TemperatureSensorLostListener extends ThingLostListener {
	
    public void notifyThingLost(final DiscoveredThing thing) {
        if (thing.getClassId().getName().equals("TemperatureSensor")) {
        	AnsiConsole.out.println(Ansi.ansi().fgMagenta()
        	        + "Temperature sensor stopped: " + thing.getDescription()
                    + " (" + thing.getId() + ")"
                    + Ansi.ansi().fgDefault());
        } else {
        	AnsiConsole.out.println(Ansi.ansi().fgBrightBlack()
        	        + "Other sensor stopped: '"
        	        + thing.getClassId().getName() + "' (" + thing.getId() + ")"
        	        + Ansi.ansi().fgDefault());
        }
    }
}