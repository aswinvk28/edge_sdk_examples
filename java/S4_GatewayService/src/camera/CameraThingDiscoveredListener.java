...
public class CameraThingDiscoveredListener extends ThingDiscoveredListener {
    private final Camera camera;
    
    public CameraThingDiscoveredListener(final Camera cam) {
        this.camera = cam;
    }
    
    public void notifyThingDiscovered(final DiscoveredThing thing) {
        if (this.camera.isRelated(thing)) {
            this.camera.discoveredRelatedCamera(thing.getId(), thing.getContextId());
        }
    }
}
