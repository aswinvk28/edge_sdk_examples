This folder contains examples on how to use the Edge SDK C++ API.

-----------------------------------------------------------------------------------------------------------------------
Examples:

The Edge SDK C+++ includes a number of examples that can be found 
in directory /examples/cpp.

- Scenario 1: Connecting a sensor (S1_ConnectSensor, S1_ConnectSensorProtobuf)
- Scenario 2: Connect a dashboard (S2_Dashboard, S2_DashboardProtobuf)
- Scenario 2A: Connect a dashboard: demonstrates usage of flowId filter (S2A_Dashboard, S2A_DashboardProtobuf)
- Scenario 3: A derived value processing service (S3_DerivedValue, S3_DerivedValueProtobuf)
- Scenario 4: A gateway service (S4_GatewayService, S4_GatewayServiceProtobuf)
- Scenario 5: Dynamic Browsing (S5_DynamicBrowsing, S5_DynamicBrowsingProtobuf)
- ThingThroughput: a throughput-tester application

Note: Examples with names ending in "Protobuf" define message formats 
using Google Protocol Buffers.

The 'Edge SDK C++ User Guide' (in the /docs directory) gives 
...
4) mkdir bld
5) cd bld
6) cmake $EDGE_SDK_HOME/examples/cpp/S1_ConnectSensor
7) make

Run the two executables produced in shells where 
...
cmake -DCMAKE_BUILD_TYPE=Debug $EDGE_SDK_HOME/examples/cpp/S1_ConnectSensor
...
5) mkdir bld
6) cd bld
7) cmake -G"NMake Makefiles" -DCMAKE_BUILD_TYPE=Release "%EDGE_SDK_HOME%\examples\cpp\S1_ConnectSensor"
8) nmake


Run the two executables produced in a command prompt where 
