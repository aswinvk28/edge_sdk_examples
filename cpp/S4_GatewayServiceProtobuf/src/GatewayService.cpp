...
public:
    DataFlowKey(const VDataSample& dataSample) :
...
private:
    ...
    Thing createThing() {
        ...
        tcr.registerThingClassesFromURI("file://definitions/ThingClass/com.adlinktech.example.protobuf/GatewayServiceThingClass.json");
...
public:
    ...
    int run(int runningTime) {
        ...
        do {
            // Read data
        	const VLoanedDataSamples msgs = m_thing.read_next("dynamicInput", (runningTime * 1000) - elapsedTime);
            ...
            for (const VDataSample& msg : msgs) {
                ...
                Observation obsData;
                CameraState camStateData;

                if(msg.isCompatible(data)) {
                    msg.get(data);
                    dataSize = data.ByteSizeLong();
                }
                else if(msg.isCompatible(alarmData)) {
                    msg.get(alarmData);
                    dataSize = alarmData.ByteSizeLong();
                }
                else if(msg.isCompatible(obsData)) {
                    msg.get(obsData);
                    dataSize = obsData.ByteSizeLong();
                }
                else if(msg.isCompatible(camStateData)) {
                    msg.get(camStateData);
                    dataSize = camStateData.ByteSizeLong();
                }

                bool sampleContainsData = (flowState == FlowState::ALIVE) || (dataSize > 0);
