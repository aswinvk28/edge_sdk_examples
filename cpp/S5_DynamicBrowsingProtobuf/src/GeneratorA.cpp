...
#include "cxxopts.hpp"
#include "FuelLevelSensor.cpp"
#include "TemperatureSensor.cpp"
#include "RotationalSpeedSensor.cpp"

static void FuelLevelSensorTask(string thingPropertiesUri, int runningTime) {
    try {
        FuelLevelSensor(thingPropertiesUri).run(runningTime);
    }
...
static void GetCommandLineParameters(int argc, char *argv[],
        string& fuelLevelSensorThingPropertiesUri, 
    ...
    try {
        ...
        options.add_options()
            ("f,fuel-sensor", "Fuel Level Sensor Thing properties URI", cxxopts::value<string>())
        ...
        if (cmdLineOptions.count("f") == 0 || cmdLineOptions.count("s") == 0 || cmdLineOptions.count("t") == 0) {
            cerr << "Please provide Thing Property URIs for all sensors" << endl;
            cerr << options.help({""}) << endl;
            exit(1);
        }
        
        fuelLevelSensorThingPropertiesUri = cmdLineOptions["f"].as<string>();
...
int main(int argc, char *argv[]) {
    ...
    string speedSensorThingPropertiesUri;
    int runningTime;
    GetCommandLineParameters(argc, argv, fuelLevelSensorThingPropertiesUri, 
            temperatureSensorThingPropertiesUri, speedSensorThingPropertiesUri, runningTime);

    // Create threads for Sensors
    thread tFuelLevelSensor(FuelLevelSensorTask, fuelLevelSensorThingPropertiesUri, runningTime);
