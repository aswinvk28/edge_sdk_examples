...
class GpsSensorDataListener : public DataAvailableListenerEx {
private:
...
public:
    ...
    void notifyDataAvailable(VLoanedDataSamples data) {
        for (auto& locationMessage : data) {
            if (locationMessage.getFlowState() == FlowState::ALIVE) {
                // Get location data from sample
                Location locationData;
                locationMessage.get(locationData);
...
private:
    ...
    Thing createThing() {
        ...
        tcr.registerThingClassesFromURI("file://definitions/ThingClass/com.adlinktech.example.protobuf/DistanceServiceThingClass.json");
