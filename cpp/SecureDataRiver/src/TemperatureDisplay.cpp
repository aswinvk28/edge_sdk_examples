...
public:
    TemperatureDisplay(string dataRiverConfigUri, string thingPropertiesUri) :
            m_dataRiverConfigUri(dataRiverConfigUri), m_thingPropertiesUri(thingPropertiesUri) {
...
int main(int argc, char *argv[]) {
    ...
    // Get DataRiver config from environment variable
    const char * riverConfKey = "ADLINK_DATARIVER_URI";
    const char * riverConfEnv = getenv(riverConfKey);
    if (riverConfEnv == nullptr) {
        cerr << "Environment variable " << riverConfKey << " not set" << endl;
        exit(1);
    }
    string riverConfigUri = string(riverConfEnv);

    try {
        TemperatureDisplay(riverConfigUri, thingPropertiesUri).run(runningTime);
    }
