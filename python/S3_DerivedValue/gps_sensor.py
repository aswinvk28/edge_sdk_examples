...
class GpsSensor(object):
    ...
    def create_thing(self):
        ...
        tcr.register_thing_classes_from_uri(get_abs_file_uri('definitions/ThingClass/com.adlinktech.example/GpsSensorThingClass.json'))
    ...
    def write_sample(self, location_lat, location_lng, timestamp):
        ...
        lat_v = IotNvp()
        lat_v.name = 'latitude'
        lat_v.value.float32 = location_lat
        lng_v = IotNvp()
        lng_v.name = 'longitude'
        lng_v.value.float32 = location_lng
        location_v = IotValue()
        location_v.nvp_seq = (lat_v, lng_v)
        
        timestamp_v = IotValue()
        timestamp_v.uint64 = timestamp
        
        sensor_data = IotNvpSeq()
        sensor_data.append(IotNvp('location', location_v))
        sensor_data.append(IotNvp('timestampUtc', timestamp_v))
