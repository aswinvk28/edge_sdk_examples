...
import os
import time
from adlinktech.datariver import DataRiver, JSonThingClassRegistry, JSonThingProperties, ThingEx, WireFormat
...
from adlinktech.datariver import DynamicTagGroupPool
...
class NewThingDiscoveredListener(ThingDiscoveredListener):
    ...
    def display_pb_field(self, field, prefix = '', width = 0):
        name = field.name
        desc = field.GetOptions().Extensions[datariver_field_options].description
        unit = field.GetOptions().Extensions[datariver_field_options].unit
        kind = pb_types[field.type]
        print(prefix + COLOR_YELLOW + '{0:<{w}}'.format(name, w=width) + NO_COLOR + ': ' + desc +
              ' (kind: {}'.format(kind) +
              ' | unit: {})'.format(unit)
              )
    ...
    def display_tag_group(self, tag_group, prefix = ''):
        ...
        print(prefix + 'Tags: ')
        try:
            if tag_group.wire_format == WireFormat.NVP_SEQ:
                type = tag_group.top_level_type
                for tag in type.tags:
                    self.display_tag(tag, prefix + '   ', 15)
            else:
                pool = DynamicTagGroupPool()
                desc = pool.descriptor_by_tag_group(tag_group)
                for f in desc.fields:
                    self.display_pb_field(f, prefix + '   ', 15)
...
class ThingBrowser(object):
    ...
    def create_thing(self):
        ...
        tcr.register_thing_classes_from_uri('file://definitions/ThingClass/com.adlinktech.example.protobuf/ThingBrowserThingClass.json')
