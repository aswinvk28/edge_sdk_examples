...
from adlinktech.datariver import DataRiver, JSonThingClassRegistry, JSonThingProperties, FlowState, BLOCKING_TIME_INFINITE, ThingEx
from definitions.ThroughputTagGroup_pb2 import Throughput
from definitions.ThroughputTagGroup_dr import Throughput_register_with_datariver
...
class ThroughputReader(object):
    ...
    def create_thing(self):
        ...
        Throughput_register_with_datariver(self._datariver)
  
        # Create and Populate the ThingClass registry with JSON resource files.
        tcr = JSonThingClassRegistry()
        tcr.register_thing_classes_from_uri('file://definitions/ThingClass/com.adlinktech.example.protobuf/ThroughputReaderThingClass.json')
    ...
    def run(self, polling_delay, running_time):
        ...
        while not stop and (running_time == 0 or cycles < running_time):
            ...
            samples = self._thing.read('ThroughputInput', BLOCKING_TIME_INFINITE)
            ...
            for sample in samples:
                if sample.flow_state == FlowState.ALIVE:
                    ...
                    if sample.is_compatible(Throughput):
                        tp_data = sample.get(Throughput)
                        # Add the sample payload size to the total received
                        received_sequence_number = tp_data.sequencenumber
                        received_data = tp_data.sequencedata
                        payload_size = len(received_data)
