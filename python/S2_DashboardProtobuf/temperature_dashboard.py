...
class TemperatureDashboard(object):
    ...
    def run(self, floor, running_time) :
        ...
        selector = self._thing.select('temperature')
        selector.flow(selection)
        print('Selecting data using flowId selector: ' + selection)
         
        while True:
            # Read data using selector
            msgs = selector.read(int((running_time - elapsed_seconds) * 1000))
            ...
            for msg in msgs:
                ...
                if flow_state == FlowState.ALIVE:
                    if msg.is_compatible(Temperature):
                        ...
                        print('Temperature data received for flow {}: {:5.1f}'.format(str(msg.flow_id), sensor_data.temperature))
